...
describe("Sidebar", () => {
  ...
  it("puts Market, Pulse and News on one line, each its own button", async () => {
    ...
    expect(buttons.map((b) => b.text())).toEqual(["Market", "Pulse", "News"]);
    ...
    for (const id of ["market", "pulse", "news"]) {
