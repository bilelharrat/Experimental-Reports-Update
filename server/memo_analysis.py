...
_FAST_MEMO_PASSES: tuple[_FastMemoPassSpec, ...] = (
    ...
    _FastMemoPassSpec(
        ...
        focus=(
            "Size the market the company actually competes in. FIRST collect "
            "every external estimate you can find — at least three when they "
            "exist: syndicated research houses (Gartner, IDC, Stratpace, "
            "Technavio, MarketsandMarkets, Grand View, Statista ...), the "
            "company's own pitched TAM, bank or analyst sizing — and record "
            "for each: who produced it, what it counts (the definition), the "
            "value, the year, the retrieval date, and the URL. NEVER discard "
            "an estimate as unusable because it disagrees with the company's "
            "revenue or with another estimate; keep it and explain the "
            "definition mismatch (a foundation-model-spend figure, a "
            "software-displacement figure, and a labor-substitution figure "
            "measure different things and can all be right). THEN build the "
            "memo's own derivation with every input named (base, growth "
            "rate, displacement or penetration factor, and where each comes "
            "from) and reconcile it against the external estimates, stating "
            "which definition the memo adopts and why. Then TAM/SAM/SOM with "
            "the derivation method for each, the market definition and "
