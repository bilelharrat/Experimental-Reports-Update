...
- 使用自然的大陆机构研究中文，避免机翻腔。
- 保留资产代码、指数名、模型名、来源机构名、专有名词、数字、日期、URL；没有通行中文译名时不要强行翻译。
- 如果英文市场术语没有自然中文对应，可保留英文并在首次出现时加简短中文解释。
- 不要把 "tailwind" 译为"顺风"；按语境使用"行业利好"、"需求侧利好"、"结构性利好"、"顺势因素"，或保留 `tailwind（利好因素）`。
- 不要机械翻译新造短语；优先翻译其分析含义，必要时保留英文短语并加解释。
- 不要把 "runway" 机械译为"跑道"，除非语境中已经是自然说法；按语境使用"现金可支撑时间"、"增长空间"或保留 `runway（可支撑时间/增长空间）`。
- 不要把 "stickiness" 按字面译成物理性质；使用"客户黏性"、"用户黏性"，或直接说明留存率与转换成本。
