...
def test_analysis_pass_prompt_carries_recent_news(tmp_path, monkeypatch):
    ...
    assert "## Recent tracked news" in captured["append_system_prompt"]
    assert "Series C" in captured["append_system_prompt"]
    captured.clear()
    kwargs["research_dir"] = tmp_path / "research" / "other-co"
    claude_runner.run_memo_fast_analysis_pass(**kwargs)
    assert "Recent tracked news" not in captured["append_system_prompt"]
