...
def test_chinese_parity_ignores_duplicate_sources_headings(tmp_path):
    """English often restates Sources (disclosures + fact index); count unique."""
    ...
    for title, body in (
        ...
        ("FINANCIAL FORECAST AND VALUATION", "No disclosed revenue."),
        ("SOURCES, SOURCE CLASSES, AND DISCLOSURES", "Primary sources listed."),
        ("Sources, Source Classes, and Fact Reference Index", "Fact index."),
        ("VI. SOURCES, SOURCE CLASSES, AND FACT REFERENCE INDEX", "Index restated."),
    ):
        en_doc.add_paragraph(title)
        en_doc.add_paragraph(body)
    for title, body in (
        ("执行摘要", "在部署深度可见时，我们建议参与。"),
        ("公司概况", "公司向整车厂出售工程产能。"),
        ("投资亮点", "认证资质支撑该投资逻辑。"),
        ("投资风险", "股权披露不完整。"),
        ("财务预测与估值", "无披露收入。"),
        ("VI. 来源、来源类别与事实索引", "来源与事实索引。"),
    ):
        zh_doc.add_paragraph(title)
        zh_doc.add_paragraph(body)
    en_doc.save(out_en)
    zh_doc.save(out_zh)

    en_shape = memo_chinese_parity._extract_docx_shape(out_en, "en")
    zh_shape = memo_chinese_parity._extract_docx_shape(out_zh, "zh")
    assert en_shape.section_ids.count("sources") == 3
    assert en_shape.section_count == zh_shape.section_count == 6
...
def test_chinese_parity_accepts_bare_section_titles(tmp_path):
    """Renderer/Claude often emit bare titles without roman numerals."""
    out_en = tmp_path / "en.docx"
    out_zh = tmp_path / "zh.docx"
    en_doc = Document()
    zh_doc = Document()
    for en_title, zh_title, body_en, body_zh in (
        (
            "EXECUTIVE SUMMARY",
            "执行摘要",
        ...
        (
            "COMPANY OVERVIEW",
            "公司概况",
            "The company sells engineering capacity to OEMs.",
            "公司向整车厂出售工程产能。",
        ),
        (
            "INVESTMENT HIGHLIGHTS",
            "投资亮点",
            "Accreditation and delivery scale support the case.",
            "认证资质与交付规模支撑该投资逻辑。",
        ),
        (
            "INVESTMENT RISK",
            "投资风险",
            "Key risk is ownership and disclosure gaps.",
            "主要风险是股权归属与披露缺口。",
        ),
        (
            "FINANCIAL FORECAST AND VALUATION",
            "财务预测与估值",
            "No disclosed revenue; valuation uses conservative proxies.",
            "无披露收入；估值使用保守代理指标。",
        ),
        (
            "VI. Sources, Source Classes, and Fact Reference Index",
            "VI. 来源、来源类别与事实索引",
            "[S1] Company materials.",
            "[S1] 公司材料。",
        ),
    ):
        en_doc.add_paragraph(en_title)
        en_doc.add_paragraph(body_en)
        zh_doc.add_paragraph(zh_title)
        zh_doc.add_paragraph(body_zh)
    en_doc.save(out_en)
    zh_doc.save(out_zh)
