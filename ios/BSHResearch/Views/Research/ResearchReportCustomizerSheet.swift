...
/// Institutional-grade research report customizer modal for iPadOS and iOS.
/// Matches macOS and Web capabilities: report archetypes, target audience calibration,
/// human-in-the-loop studio workflow vs one-click autonomous pipeline, frontier reasoning tiers,
/// freeform directives with suggestion chips, strategic risk pillars, company sector lenses,
/// and evidence cross-referencing.
public struct ResearchReportCustomizerSheet: View {
    public let initialCompany: MacCompany?
    public var onGenerated: ((MacReport) -> Void)? = nil

    @EnvironmentObject private var store: ResearchDeskStore
    @Environment(\.dismiss) private var dismiss
    @Environment(\.horizontalSizeClass) private var horizontalSizeClass

    public enum CustomizerTab: String, CaseIterable, Identifiable {
    ...
    private var currentCompany: MacCompany? {
        selectedCompany ?? initialCompany ?? store.companies.first
    }

    public init(company: MacCompany, onGenerated: ((MacReport) -> Void)? = nil) {
        self.initialCompany = company
        self.onGenerated = onGenerated
        _selectedCompany = State(initialValue: company)
    }

    public init(company: MacCompany? = nil, onGenerated: ((MacReport) -> Void)? = nil) {
        self.initialCompany = company
        self.onGenerated = onGenerated
        _selectedCompany = State(initialValue: company)
    }
            ...
            VStack(alignment: .leading, spacing: 3) {
                HStack(spacing: 6) {
                    if initialCompany != nil {
                        Text(currentCompany?.name ?? currentCompany?.id ?? "Company")
                            .font(isCompact ? .headline : .dsTitle)
                            .foregroundStyle(.primary)
                            .lineLimit(1)
                    } else {
                        // Company Selector Menu
                        Menu {
                            ForEach(store.companies) { comp in
                                Button {
                                    selectedCompany = comp
                                } label: {
                                    HStack {
                                        Text(comp.name ?? comp.id)
                                        if let ticker = comp.ticker {
                                            Text("(\(ticker))")
                                        }
                            ...
                            HStack(spacing: 4) {
                                Text(currentCompany?.name ?? currentCompany?.id ?? "Select Company")
                                    .font(isCompact ? .headline : .dsTitle)
                                    .foregroundStyle(.primary)
                                    .lineLimit(1)
                    ...
                    if let ticker = currentCompany?.ticker, !ticker.isEmpty {
                    ...
                    if let sector = currentCompany?.sector ?? currentCompany?.industry, !sector.isEmpty {
                        Text(sector)
                            .font(.system(size: 10))
                            .padding(.horizontal, 6)
                            .padding(.vertical, 2)
        ...
        VStack(alignment: .leading, spacing: 20) {
            ...
            if isCompact {
                ...
                HStack(alignment: .top, spacing: 16) {
                    scopeSection.frame(maxWidth: .infinity)
                    languageSection.frame(maxWidth: .infinity)
                }
            }
        }
    }
    ...
    private var directivesTabContent: some View {
        VStack(alignment: .leading, spacing: 20) {
            ...
            VStack(alignment: .leading, spacing: 8) {
                MacSectionLabel("ANALYST DIRECTIVES & FOCUS QUESTIONS", trailing: "Freeform guidance provided directly to research agents")
            ...
            VStack(alignment: .leading, spacing: 8) {
                ...
                .background(Color.dsCard)
                .clipShape(RoundedRectangle(cornerRadius: 8, style: .continuous))
                .overlay(RoundedRectangle(cornerRadius: 8, style: .continuous).stroke(Color.dsHairline, lineWidth: 1))
                .frame(maxWidth: 320)
            }
        }
    }
    ...
    private var sourcesTabContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            let reports = (currentCompany?.id).map { store.reports(for: $0) } ?? []
            if reports.isEmpty {
                VStack(spacing: 12) {
                    Image(systemName: "doc.badge.plus")
                        .font(.system(size: 32))
                        .foregroundStyle(.tertiary)
                    Text("No prior research dossiers for \(currentCompany?.name ?? currentCompany?.id ?? "company") yet.")
                        .font(.dsSubhead)
                        .foregroundStyle(.secondary)
                ...
                .background(Color.dsCard)
                .clipShape(RoundedRectangle(cornerRadius: MacDS.tileRadius, style: .continuous))
                .overlay(RoundedRectangle(cornerRadius: MacDS.tileRadius, style: .continuous).stroke(Color.dsHairline, lineWidth: 1))
            } else {
                VStack(spacing: 8) {
    ...
    private var bottomActionBar: some View {
        VStack(spacing: 10) {
            ...
            HStack(spacing: 12) {
                ...
                .controlSize(.regular)
                .disabled(isSubmitting || currentCompany == nil)
            }
        }
        .padding(.horizontal, isCompact ? 16 : 24)
        .padding(.vertical, 12)
...
public typealias MacGenerateReportSheet = ResearchReportCustomizerSheet
