...
/// The monumental mountain watermark rising behind the canvas, fading softly upwards.
public struct BSHSummitWatermark: View {
    public var width: CGFloat = 620
    @Environment(\.colorScheme) private var colorScheme

    public init(width: CGFloat = 620) {
        self.width = width
    }
    ...
    public var body: some View {
        ...
        BSHBrandMarkShape()
            .fill(
                LinearGradient(
                    colors: [
                        Color.accentColor.opacity(isDark ? 0.06 : 0.04),
                        Color.primary.opacity(isDark ? 0.04 : 0.025)
                    ],
                    startPoint: .top,
                    endPoint: .bottom
                )
