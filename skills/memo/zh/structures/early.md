---
en_sha256: 4ef1568828b116aa9af571b049a7ebce600fd34c5d1744e4a31169b5351cdaa3
---
---
stage: early
version: 1
...
500-700 词。执行摘要唯一的任务是说清什么最重要。它不含任何表格——它
需要的每一个数字都放在一句解释该数字的句子里（Key Facts 表放在"创始
团队与公司"章节，Deal Terms 表放在"交易条款与回报测算"章节）。各小节
内容：
...
3. 核心投资亮点：以共享事实表中已钉定的案例总结句开头，用公司名称
   ...
   每条对应一个已钉定亮点，按钉定顺序：先是已钉定的标题原文（加粗
   呈现），再是已钉定的证据句，每个数字在读者看到它之前都先交代它
...
5. 投资结论与建议：一个结论提示框——已钉定的投资建议句原文、入场
   ...
   `decision_history_sentence` 在提示框之后作为历史原文陈述。
...
2. 风险卡片：4-6 张按风险逐一展开的卡片，按评分从高到低排序，严格
   ...
   key_value 表，行为：风险类型 | 为什么重要（事实 → 失败模式 →
   后果）| 跟踪信号（在这一阶段：消除该风险的里程碑——可观察的证明点
   及其预期日期，因为下一轮定价看的正是它）| 缓释措施（真实的机制
   ——公司行动、交易结构或仓位控制；若不存在："No structural
   mitigation exists. <consequence>"）| 可能性 | 风险评分 N/10。
