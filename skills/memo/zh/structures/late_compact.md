---
en_sha256: fc60914a3f99c3b43e337734e7a5db983c5c246576a43b25acbe5271467bed99
---
---
stage: late_compact
version: 1
...
（pin-echo）关卡会拒收整个报告包，无论文笔多么优雅。已钉定事实每章只写
一次：归属某个钉定事实的章节在归属它的那段里逐字写一次，其他任何提及 ——
无论在本章还是别的章节 —— 都只指代它，不再重复数字（"2026 年 2 月的估值
标记"、"基准情形"）。每个要求章节里一次逐字陈述是底线，也是上限：本模板
的一次实际运行把入场估值重复了四十九次、把商业合计金额重复了三十五次，
仅风险章节就靠这些重复写到了 5,300 词。章节字数预算是
...
3. 核心投资亮点：
   ...
   Tarnwell Robotics 是一家虚构公司：学它的形态，绝不照搬其中的事实或
   措辞；每条证据句说明"这个数字衡量什么"的方式也要换着写：

   ```
   Tarnwell 这笔投资主要由市场空间与增速（13/15）、行业地位（12/15）
   和护城河（11/15）支撑；最单薄的是商业模式与单位经济（4/10）和估值
   （5/10）。

   "component": "investment_highlights"

   [market_size_growth]
   决定这个市场规模的是劳动力缺口，而不是技术，而且缺口还在扩大。
   一项行业协会调查统计，2025 年美国冷库叉车操作岗位约有 38,000 个空缺
   ——这个数字说明机器人可以接手多少岗位而不挤掉任何人[S6]。据我们自己的
   估算，可服务的车队规模为每年 $9-11B，由这一空缺数与每台机器人的年租赁
   价格推算得出[C3]。

   [industry_position]
   这些机器人已经在最难的通道里干活 —— 能在冷冻库里跑无人值守班次的自主
   叉车厂商本就寥寥，规模做到这一步的一家也没有。
   按公司自己的车队日志，无人驾驶运行时长从 2025 年的约 40,000 小时升至
   2026 年上半年的 310,000 小时[S4]。其中约 70% 的时长发生在冷冻库，而
   冷冻库正是最先招不到人的地方[S4]。

   [moat]
   分发不必自建，因为三大叉车经销网络已经在代售这些机器人 —— 仓储运营方
   可以通过一个已经通过审批的供应商直接采购。
   经销协议已覆盖美国三大叉车经销网络，也就是大多数运营方本来就在用的
   采购渠道；最后一份于 2026 年 6 月签署[S9]。经销商承担了约 55%-60% 的
   新增台数——这是我们根据已披露的渠道拆分做出的判断，也衡量了订单中有多少
   依赖这些渠道[C7]。
   ```
...
5. 投资结论与建议：一个结论标注框（callout）—— 已钉定的投资建议句
   ...
   `decision_history_sentence` 在标注框之后作为历史记录逐字陈述。然后，
   作为本章的最后一个块，输出 "Terms used"（术语说明）：一个 `glossary`
   块，`component: "glossary"`，其 `items` 为本备忘录用到的每个专业术语
   一项（MOIC、IRR、ARR、NRR、CAGR、TAM/SAM/SOM、run-rate、post-money、
   MOU 等 —— 6-15 项），每项形如 `{"term": {"en", "zh"}, "definition":
   {"en", "zh"}}`，定义五到十个词。这是所有这些术语唯一的定义之处：
   其他章节不再定义它们。
