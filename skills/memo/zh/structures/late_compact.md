---
en_sha256: deff3bb6ccd98027ad55b00de7b29e83be1496b4f6a340da333f04a76972840b
---
---
stage: late_compact
version: 1
...
450-600 词。不得使用表格。各子节内容：

1. 项目基本概况：用一句平实的话说明公司做什么、为谁做；一句说明
   行业/地域；一句说明其在后期融资阶梯上的位置。不超过 60 词。
2. 本轮融资方案：轮次、工具、规模、价格、隐含持股比例，以及这个价格
   已经预设了什么 —— 入场倍数须在同一口气中给出解读。2-3 句。
3. 核心投资亮点：以共享事实表中已钉定的案例总结句开头，用公司名称
   替换 "The case" —— 说明哪些维度支撑本案、哪些维度较弱，并附分数。
   然后是一个 `bullets` 块，`"component":
   "investment_highlights"`，恰好三项，按钉定顺序排列：已钉定的标题
   逐字原文（渲染为粗体），随后是已钉定的证据句，每个数字都先说明它
   衡量的是什么、来自哪里（一个具名来源，或 "the memo's own estimate"
   及其输入）。绝不允许出现裸露的主题标签（"Price."），绝不允许新增
   数字。
...
2. 市场空间与天花板：首先，调研找到的每一个外部估计各占一条要点
   ...
   （规模、年份，以及所依据的外部估计或本备忘录自己的输入 —— 或缺失
