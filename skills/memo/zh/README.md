...
**报告代理实际读取的是英文版**；中文版是给中文团队阅读、修改用的。
...
zh/
  ...
  buffett.md               巴菲特方法备忘录的写作要求（BSH 研究的所有者分析，单次运行，不属于晚期报告的 pass）
  zh_style.md              中文写作规范与术语表（| 英文 | 中文 | 避免 |）——每个撰写备忘录
                           中文的提示词都会带上它；改这里，同步后即生效
  jurisdictions/           按司法辖区的调研补充（cn.md：各 pass 的中文检索词），
                           识别出该辖区时加入各 pass 的共享上下文
```
...
代理实际使用的术语表在 `zh_style.md`（改那里的中文版，同步后生效）。下表是原有的
对照表；两者不一致的地方以 `zh_style.md` 为准，并已在其中标注"待团队确认"。
